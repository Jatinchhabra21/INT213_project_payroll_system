...
class Application(Frame):
    ...
    def searchEmp(self, event=None):
        try:
            ...
            mb.showerror(title='Error!', message='Enter a Valid ID or Name.')
        if (self.dat == None):
            mb.showerror(title='Not Found', message='Invalid Input')
            return
        elif (not self.dat):
            mb.showwarning(title='Not Found', message='The instance doesn\'t exist in database.')
            return
    ...
    def createWidgets(self):
        ...
        # Search Button
        self.search_button = Button(f2, text='Search', width=31, bd=2, relief=FLAT, font=('Calibri', 15),
                                    activeforeground='white', highlightbackground='lightblue', highlightcolor='white',
                                    bg='white', command=self.searchEmp)
        self.search_button.place(x=90, y=370)

        # Add new Employee Button
        self.add_button = Button(f2, text='Add New Employee', width=31, bd=2, relief=FLAT, activeforeground='white',
                                 highlightbackground='lightblue', font=('Calibri', 15),
                                 highlightcolor='white', command=self.addEmp)
        self.add_button.place(x=90, y=400)

        self.summaryButton = Button(self.master, text='Graph Summary', width=31, bd=2, relief=RAISED,
                                    font=('Calibri', 15),
                                    highlightbackground='lightblue', highlightcolor='white', activeforeground='white',
